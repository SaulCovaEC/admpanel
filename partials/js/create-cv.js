((d, c) => {
  /*----- Variables -----*/
  let sendCv = d.getElementById("cv-form");
  ...
  let modalLoading = d.getElementById("modal-loading");
...
function reasignarIds(element) {
  ...
  for (let i = 0; i < deleteElement.length; i++) {
    ...
    function showContentModal(status) {
        modalLoading.setAttribute("class", 'hide');
        modalContent.setAttribute("class", '');
        
        if(status == 'success') {
            modalBody.innerHTML = successStatus();
        } else {
            modalBody.innerHTML = errorStatus();
        }
    }
...
function nuevoCv() {
  ...
  for (let index = 0; index < additionalsReg.length; index++) {
    ...
    .then(function(response){
        (response) ? showContentModal("success") : showContentModal("error");
    });
}
...
if(sendCv){
  sendCv.addEventListener("submit", function(event){
    event.preventDefault(); 
    nuevoCv();
  });
}
